...
select no_plan();
insert into auth.users(id,email) values('00000000-0000-0000-0000-00000000ac01','member-sort@test.local');
update profiles set role='admin' where user_id='00000000-0000-0000-0000-00000000ac01';
...
select set_config('request.jwt.claims','{"sub":"00000000-0000-0000-0000-00000000ac01","role":"authenticated"}',true);
...
select is((select members-(select members from dues_baseline) from subscription_dues_summary(2090)),
 4,'four members owe');
select lives_ok($$select record_subscription_payment(84002,2090,500,'2090-06-01',null,gen_random_uuid())$$,
...
select lives_ok($$select record_subscription_payment(84003,2090,200,'2090-06-01',null,gen_random_uuid())$$,
